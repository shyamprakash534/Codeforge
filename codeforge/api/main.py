...
app = FastAPI(title="CodeForge API", version="1.3.0")
_runs = 0
_successes = 0
...
def _public_github_preview(repo: str, request: str) -> dict:
    """Analyze a public repo without executing its code or modifying it.

    Render is a public service, so arbitrary GitHub repositories must not be
    allowed to execute their tests/build scripts. The full autonomous workflow
    remains available for trusted local workspaces.
    """
    ...
    try:
        ...
        researcher = ResearcherAgent(scanner, retrieval)
        state = researcher.run(state, tracer)
        state.phase = TaskPhase.DONE
        state.approval_status = state.approval_status.PENDING
        state.pr_metadata = {
            "preview": True,
        ...
        result["note"] = "For full code changes, test execution, repair, and PR creation, use a trusted local workspace."
        return result
    finally:
...
<section class="panel"><h2>Run an engineering task</h2><p class="hint">Public GitHub URLs use a safe analysis-only preview. Local workspaces can run the full engineering workflow.</p><div class="grid"><div class="field"><label for="repo">GitHub repository URL or local path</label><input id="repo" placeholder="https://github.com/owner/repository or ." aria-label="Repository"></div><div class="field"><label for="request">Task</label><textarea id="request" placeholder="Example: Analyze this repository and add a health-check test." aria-label="Task"></textarea></div></div><div class="actions"><button id="run" type="button">Run CodeForge</button><span class="status" id="status">Ready</span></div><pre class="output" id="out">Workflow output will appear here.</pre></section>
...
def run(req: RunRequest):
    ...
    try:
        ...
        if result.get("result") in {"ANALYSIS_COMPLETE"} or result.get("phase") == "DONE": _successes += 1
        return result
    ...
    except subprocess.TimeoutExpired: raise HTTPException(status_code=504, detail="Repository clone timed out after 90 seconds")
    except RuntimeError as exc: raise HTTPException(status_code=502, detail=str(exc))
    except Exception as exc: raise HTTPException(status_code=500, detail=f"CodeForge execution failed: {type(exc).__name__}")
"